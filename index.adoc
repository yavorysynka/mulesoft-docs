...
MuleSoft is a cloud software company helping organizations transform existing data and applications to proactively adapt to changing needs in their fields.
...
A Mule application is a configuration of functional elements in a *Mule flow* to process data in a sequence. Mule runtime passes important data through the Mule app flow using a *Mule message*. It is up to the integration developer to design the flow and configure how its elements operate on this message. Many elements can be configured to hook into existing applications on-premises or in the cloud to perform desired operations on data.

Once a Mule app is deployed on Mule runtime, the app's main flow is ready to be kicked off by the event that triggers it. You can deploy Mule applications to Mule runtime servers anywhere, and configure security and policies for them as well. In Mule apps *you write less custom code, and get more done through reuse.*

[NOTE]
Consider API-led connectivity when connecting assets. APIs allow the developer to modify assets and redeploy them without having to change the APIs that consume the assets.
...
The Anypoint Platform tools are accessed primarily through a browser but also have command line tools. Through the platform it is possible to control most aspects of integrations applications and APIs through their entire lifecycle.


== Anypoint Platform Tools

Follow these steps to get an Anypoint Platform account and check out the tools.
...
. link:/anypoint-studio/v/6/download-and-launch-anypoint-studio[Download and Install Anypoint Studio with Mule].
...
. Check out templates, connectors and more from link:/getting-started/anypoint-exchange[Anypoint Exchange], where you can find and publish developer resources. You can use Exchange to publish private repositories.

[NOTE]
If working with Mule outside Studio or have an enterprise license see how to link:/mule-user-guide/v/3.8/configuring-maven-to-work-with-mule-esb[Configure Maven] to properly manage dependencies used in your Mule apps.
...
For *Mule deployment*, see link:/runtime-manager/deployment-strategies[deployment strategies]. That document explains the standard app-to-runtime deployment workflows and tools.
...
[TIP]
Find out how to https://docs.mulesoft.com/runtime-manager/managing-servers#add-a-server[Register a Mule runtime instance] with your Anypoint Platform instance.

For *Anypoint Platform deployment*, the options are the MuleSoft hosted version, and the containerized
link:https://docs.mulesoft.com/anypoint-platform-on-premises/v/1.5.0/[Anypoint Platform On-Prem], to deploy the platform to your infrastructure on-premises.
