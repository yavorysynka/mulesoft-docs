...
MuleSoft is a cloud software company helping organizations transform existing data and applications to proactively adapt to changing needs in their fields. Anypoint Platform is MuleSoft's *integration application and API suite*.

== What is Mule?

Basically, Mule is an engine for processing and orchestrating data. Mule is the layer that allows you to connect diverse systems reliably. Mule is a Java-based enterprise service bus (ESB) based on SEDA (staged-event driven architecture). Using Mule you can build enterprise-grade integration applications called Mule applications. Traditionally it was called Mule ESB, but is now more commonly referred to as Mule runtime.

== What is a Mule application?

A Mule application links up functional elements in a *Mule flow* to process data. Mule passes data through the flow inside a *Mule message*. It is up to the integration developer to design the flow and configure how its elements operate on this message. Many elements can be configured to hook into existing applications on-premises or in the cloud to perform desired operations on data.

Once a Mule app is deployed, its main flow is ready to be kicked off by an event that triggers it. You can deploy Mule applications to Mule runtime servers anywhere, and configure security and policies for them as well. In Mule apps *you can avoid having to write custom code*.
...
Anypoint Platform comprises a unique toolset that enables organizations to create, integrate, model, build, and deploy services, APIs, and Mule applications.

The Anypoint Platform tools are primarily browser-based environments. Through the various user interfaces in the tools you can work on most aspects of integrations applications and APIs through their entire lifecycle.


== Anypoint Platform Tools


The best way to learn is to access the cloud version of Anypoint Platform, follow one of the getting started checklists, then try out the tutorials in the link:/getting-started[Getting Started with Anypoint Platform] section.
...
[NOTE]
link:https://docs.mulesoft.com/anypoint-platform-on-premises/v/1.5.0/[Anypoint Platform On-Prem] is available if you want to deploy the platform to your infrastructure on-premises.
...
. Install Anypoint Studio, which comes with Mule runtime. If you only want to work with APIs, but not Mule apps, you do not need Studio.
+
[NOTE]
...
[NOTE]
You can use a variety of tools to sync Mule projects, APIs and assets between teams.


[[getting-started-deploy]]
=== Deployment Toolkit

After Mule applications and APIs have been tested, deploy them locally or to a cloud server.

. Choose a deployment model:
- *Mule runtime on-prem* (not managed by MuleSoft)
- *CloudHub* - Mule runtime in MuleSoft's cloud
...
. Choose a management tool for your apps
- link:/runtime-manager[Runtime Manager], which is part of the Anypoint Platform
[NOTE]
Also use Runtime Manager to deploy a Mule app to CloudHub.
- link:/mule-management-console/v/3.8[Mule Management Console], a legacy deployment product
- Mule runtime managed and/or monitored by third party applications (for example: Nagios, Zabbix)
. Manage APIs with link:/api-manager[API Manager]
. Configure access to Anypoint Platform through the link:/access-management[Access Management UI]
...
You can run your apps on servers in the *cloud*, *on-premises*, or some *hybridized* version. Manage and monitor legacy or platform APIs. Manage API consumption via auto-generated proxy.
