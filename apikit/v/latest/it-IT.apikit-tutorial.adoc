...
Using the implementation URI, you can now simulate calls to the API using API Console. Per esempio:
...
+
. In a browser, go to the IP address and port shown in the implementation URI. Per esempio:
+
...
`API calls exceeded`
+
. In a browser, go to the implementation URI for the test-policy API. Per esempio:
+
