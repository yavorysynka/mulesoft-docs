...
Using the implementation URI, you can now simulate calls to the API using API Console. Zum Beispiel:
...
+
. In a browser, go to the IP address and port shown in the implementation URI. Zum Beispiel:
+
...
`API calls exceeded`
+
. In a browser, go to the implementation URI for the test-policy API. Zum Beispiel:
+
