...
== To Create a Custom PIP

Creating a custom PIP requires a DTD and an XSD file. You can load
the DTD into software that creates the XSD. Information is available
on the web on how to create both files. 
...
. Add your DTD and XSD files to the PIP folder you want to add
to. For example, to create a custom purchase order as a 3A4 PIP, 
add your custom files to the PIP3A4 folder.
. Recompile your files back into a JAR. For more information, 
see link:http://stackoverflow.com/questions/3350545/how-do-you-recompile-a-jar-file[How do you recompile a jar file?] at StackOverflow.
. Replace the old Rosetta JAR with your new one.

== About Use Case Examples

In the following example, you can build a simplified Mule application for 
a buyer and seller in Anypoint Studio. This application sends and receives 
a purchase order request and a purchase order confirmation.
